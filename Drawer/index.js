...
export default class Drawer extends Component {
	...
	componentWillUpdate(nprop, nstate) {
		const { drawerPosition } = nprop
		// console.log(drawerPosition, this.props.drawerPosition);
		

		if (drawerPosition !== this.props.drawerPosition) {
			this.isLeft = drawerPosition === positions.Both || drawerPosition === positions.Left;
			this.isRight = drawerPosition === positions.Both || drawerPosition === positions.Right;
			this.forceUpdate()
			this._updateNativeStyles(1)
			this.closeDrawer()
		}
	...
	_touchPositionCheck(gestureState) {
		...
		if (!leftDisabled && this.isLeft && x0 <= width * 0.05 && !isOpen && dx > 0) {
			this.isLeftActive = true;
			return true;
		}
		// swipe left to open right drawer
		if (!rightDisabled && this.isRight && x0 >= (this.MAX_DX + width*0.15 )&& !isOpen && dx < 0) {
